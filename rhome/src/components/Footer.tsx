...
export default function Footer() {
  return (
    <footer className="border-t border-rhone-200 bg-rhone-900 text-rhone-100">
      <div className="mx-auto max-w-7xl px-6 py-16">
        <div className="grid grid-cols-2 gap-10 md:grid-cols-5">
          <div className="col-span-2">
            <span className="font-display text-2xl text-rhone-50">Rhone</span>
            <p className="mt-3 max-w-xs text-sm text-rhone-300">
          ...
          {columns.map((col) => (
            <div key={col.title}>
              <h3 className="text-sm font-semibold text-rhone-200">
                {col.title}
              </h3>
              <ul className="mt-4 space-y-3">
                {col.links.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="text-sm text-rhone-300 transition hover:text-rhone-50"
                    >
                      {link.label}
                    </Link>
                  </li>
        ...
        <div className="mt-12 border-t border-rhone-700/60 pt-8 text-xs leading-relaxed text-rhone-400">
          <p>
            Rhone is a telehealth platform that facilitates connections between
            patients and independent, licensed clinicians. Rhone does not
          ...
          <p className="mt-4">
            © {new Date().getFullYear()} Rhone Health, Inc. All rights
            reserved.
          </p>
        </div>
      </div>
