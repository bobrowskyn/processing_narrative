...
export default function IntakeWizard() {
  ...
  if (submitted) {
    ...
    return (
      <div className="rounded-3xl border border-rhone-200 bg-rhone-50 p-10 text-center">
        <span className="text-4xl">✓</span>
        <h2 className="font-display mt-4 text-2xl text-rhone-900">
        ...
        <p className="mt-6 text-xs text-rhone-500">
  ...
  return (
    <div className="rounded-3xl border border-rhone-200 bg-rhone-50 p-8 sm:p-10">
      <div className="mb-8 flex items-center gap-2">
        {Array.from({ length: totalSteps }).map((_, i) => (
          <div
            key={i}
            className={`h-1.5 flex-1 rounded-full ${
              i <= step ? "bg-rhone-500" : "bg-rhone-200"
            }`}
          />
        ))}
      </div>
      ...
      {step === 0 && (
        <div>
          <h2 className="font-display text-2xl text-rhone-900">
            ...
            {treatments.map((t) => (
              <button
                ...
                className={`rounded-2xl border px-5 py-4 text-left text-sm font-semibold transition ${
                  category === t.slug
                    ? "border-rhone-600 bg-rhone-100 text-rhone-900"
                    : "border-rhone-200 bg-rhone-50 text-rhone-800 hover:border-rhone-400"
                }`}
              >
                {t.name}
              </button>
      ...
      {step === 2 && (
        <div>
          ...
          <textarea
            ...
            rows={5}
            className="mt-6 w-full rounded-xl border border-rhone-200 bg-white px-4 py-3 text-rhone-900 outline-none focus:border-rhone-500"
      ...
      <div className="mt-8 flex items-center justify-between">
        <button
          ...
          onClick={back}
          disabled={step === 0}
          className="text-sm font-semibold text-rhone-600 disabled:opacity-0"
        >
          ← Back
        </button>
        <button
          ...
          className="rounded-full bg-rhone-600 px-7 py-3 text-sm font-semibold text-rhone-50 transition hover:bg-rhone-700 disabled:cursor-not-allowed disabled:bg-rhone-300"
        >
